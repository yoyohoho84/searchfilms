...
class FilmSearchContainer extends Component {
    ...
    onChangeHandler = (e) => {
        ...
        if (value !== '') {
            currentList = films;
            newArray = currentList.filter((item) => {
                return item.title.toUpperCase().indexOf(value.toUpperCase()) !== -1
            });
        } else {
            console.log('empty srtring');
            newArray = films;
        }
        this.setState({
            filtered: newArray
    ...
    findCorrect = (condition, array) => {
        const result = array.find(e => e.title === condition);
        if (result) {
            console.log(result)
        } else {
    ...
    pickFavorite = (value) => {
        ...
        const result = this.findCorrect(value, this.state.films);
        const check = this.findCorrect(value, newFavorites);

        // Записываем данные в LocalStorage
        // let data = [];
        // let saved = localStorage.getItem('data'); // попытались по ключу вытащить данные
        // if (saved !== null) { // если они там действительно есть
        ...
        if (result === check) {
            ...
            this.setState({
                favorites: newFavorites.filter(item => item !== result)
    ...
    render() {
        const {isTab1, isTab2, tags, films, filtered, favorites, bookmarksActive, bookmarksPassive} = this.state;


        const renderFilms = filtered.map((item, index) => {
            const {title} = item;
            return (
                <FilmItem1
                    id={index}
                    title={title}
                    handleClick={this.pickFavorite}
                />
            )
        });
